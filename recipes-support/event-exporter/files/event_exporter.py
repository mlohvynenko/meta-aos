...
Which log lines count as checkpoints, and (for --report-timing) which pairs of them define each
timing metric, are both defined by one YAML config file (see --config, load_config()), not
hardcoded, so this can be pointed at other services' log/checkpoint conventions without a code
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="path to a YAML file listing the checkpoint regexes to match and (for --report-timing) "
        "the checkpoint_ranges timing table, see event-exporter.yml",
    )
    parser.add_argument(
        "--unit",
        action="append",
...
_RANGE_REQUIRED_KEYS = {"label", "start_source", "start_event", "end_source", "end_event"}
_RANGE_OPTIONAL_KEYS = {"nearest_end", "optional_all"}


def _load_checkpoint_ranges(raw_ranges):
    """Validates the "checkpoint_ranges" section of the config file (see load_config()) and returns
    it as a list of dicts ready for RangeJob(**range_def) - keys match RangeJob.__init__()'s own
    parameter names exactly, so no translation layer is needed between the two. Fails fast on a
    missing/unknown key rather than letting a typo surface later as a silently-wrong or permanently
    n/a metric, since this only runs once, at startup."""
    checkpoint_ranges = []
    for i, entry in enumerate(raw_ranges):
        keys = entry.keys()
        missing = _RANGE_REQUIRED_KEYS - keys
        if missing:
            raise ValueError(f"checkpoint_ranges[{i}] ({entry.get('label', '?')!r}) missing required key(s): "
                              f"{sorted(missing)}")

        unknown = keys - _RANGE_REQUIRED_KEYS - _RANGE_OPTIONAL_KEYS
        if unknown:
            raise ValueError(f"checkpoint_ranges[{i}] ({entry['label']!r}) has unknown key(s): {sorted(unknown)}")

        checkpoint_ranges.append(entry)

    return checkpoint_ranges


def load_config(config_path):
    """Load event-exporter's one YAML config file: the checkpoint regexes matched against journald
    lines ("patterns" - see match_checkpoint()) and, for --report-timing, the checkpoint-pair table
    each timing metric is resolved from ("checkpoint_ranges" - see RangeJob and the module
    docstring's checkpoint table). Both live in one file, not two, since they describe the same
    underlying thing - which of a service's log checkpoints matter, and how - just for two different
    purposes: forwarding them live as events, versus resolving elapsed time between pairs of them
    after the fact. Returns (patterns, checkpoint_ranges)."""
    with open(config_path) as f:
        config = yaml.safe_load(f) or {}

    patterns = [re.compile(pattern) for pattern in config.get("patterns", [])]
    checkpoint_ranges = _load_checkpoint_ranges(config.get("checkpoint_ranges", []))
    return patterns, checkpoint_ranges
...
#     used to have a bounded wait. So every RangeJob metric (see resolve_suite()'s bounded_labels)
#     instead gets a bounded wait (BOUNDED_WAIT_SECONDS) - reported as n/a once it runs out - while
#     "Total" (the checkpoint that defines the suite in the first place, and so always present by
#     the time a suite is even detected - see wait_for_new_suite()) still waits indefinitely.
#
...
# The checkpoint-pair table itself (Download/Install/Prepare/Init SM/... - see the module
# docstring's checkpoint table) is config-driven now, not hardcoded here - see load_config() and
# _load_checkpoint_ranges(). START_INSTANCES_LABEL stays a Python constant (rather than moving into
# config too) because the per-instance "Start instances" breakdown in resolve_suite() needs to find
# that specific RangeJob by label at runtime; whichever config entry is meant to drive that
# breakdown must use this exact label.
...
# instead - except resolve_suite()'s own bounded_labels, see BOUNDED_WAIT_SECONDS below.
...
# Every RangeJob metric label loaded from config, plus "Total" (see resolve_suite()'s own
# bounded_labels, computed from the loaded checkpoint_ranges) - resolve_suite() gives up on each of
# these (reporting n/a) after BOUNDED_WAIT_SECONDS rather than waiting forever, since any of them
# can be genuinely absent depending on which kind of run is being watched - see the
# checkpoint-pair-applicability note above. "Total" needs "Process desired status", which - like
# Download/Install/Prepare - only fires for a genuine desired-status deployment, not a plain
# aos.target restart, so it needs the same bounded treatment. What does keep resolve_suite() waiting
# indefinitely is TotalJob's `end_us` - the last instance's own Start, the checkpoint that defines
# the suite in the first place (see wait_for_new_suite()) - which is always present by the time
# resolve_suite() starts and isn't gated by bounded_labels at all, see TotalJob.
# BOUNDED_WAIT_SECONDS is wide enough to comfortably clear VictoriaMetrics' default ~30s
...
class RangeJob:
    """Resolves one of config's "checkpoint_ranges" entries' elapsed times into results[label] - see
    load_config()."""
...
class TotalJob:
    ...
    resolve_suite()'s own bounded_labels, since it only fires for a genuine desired-status
    deployment, not a plain aos.target restart.
...
def resolve_suite(victoria_url, node, not_before_us, suite_end_us, checkpoint_ranges):
    ...
    as n/a instead once BOUNDED_WAIT_SECONDS passes. A metric outside `bounded_labels` that AosCore
    ...
    `checkpoint_ranges` is the "checkpoint_ranges" list loaded from config (see load_config()) -
    one RangeJob per entry, plus the always-present TotalJob.
...
_NON_METRIC_RESULT_KEYS = {"instances", "instance_start_durations"}
...
def _debug_metrics(results):
    ...
    report_timing_loop(). Every key in `results` other than _NON_METRIC_RESULT_KEYS' own
    list-valued entries is a resolved elapsed-time metric (RangeJob labels, config-driven, plus
    "Total") - no need to know the config's own checkpoint_ranges here just to name them."""
    parts = [
        f"{label}={value:.3f}s" for label, value in results.items() if label not in _NON_METRIC_RESULT_KEYS
    ]
...
def main():
    ...
    if args.report_timing:
        ...
        threading.Thread(
            target=report_timing_loop, args=(args.victoria_url, args.node, checkpoint_ranges), daemon=True
        ).start()
